...
async function main() {
  ...
  const upgraded = await upgrades.upgradeProxy(content.TimelockProxyAddress, Timelock);

  console.log("Timelock is upgraded.");

  const implHex = await ethers.provider.getStorageAt(
    content.TimelockProxyAddress,
    "0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc"
  );
  const implAddress = ethers.utils.hexStripZeros(implHex);

  console.log('Timelock New Impl Address: ', implAddress);
