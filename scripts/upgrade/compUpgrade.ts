...
async function main() {
  ...
  const upgraded = await upgrades.upgradeProxy(content.CompProxyAddress, Comp);

  console.log("Comp is upgraded.");

  const implHex = await ethers.provider.getStorageAt(
    content.CompProxyAddress,
    "0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc"
  );
  const implAddress = ethers.utils.hexStripZeros(implHex);

  console.log('Comp New Impl Address: ', implAddress);
